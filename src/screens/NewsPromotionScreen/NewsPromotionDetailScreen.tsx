    ...
    <Container>
      ...
      <Content
        ...
        <ScrollView
          ...
          <DashedLine dashColor={colors.border1} dashGap={0} dashLength={8} />

          {new Date(data?.startDate).getTime() <= new Date().getTime() ? (
            <>
              <View style={{ marginVertical: 20 }}>
                <Text>{data?.promotionDetail}</Text>
              </View>
              <View>
                <LinearGradient
                  style={[
                  ...
                  <View style={styles.content}>
                    <Text color="white" semiBold>
                      {`1. ${promotionTypeMap(data?.promotionType)} - ${
                        data?.promotionName
                      }`}
                    </Text>
                        ...
                        if (condition.typeMix === 'Quantity') {
                          return (
                            <>
                                ...
                                {condition.products.map(
                                  (p, idx) =>
                                    `${p.productName} ${p.packSize}${
                                      idx + 1 === condition.products.length
                                        ? ''
                                        : ','
                                    } `,
                                )}
                              ...
                              <View key={index}>
                                {condition?.conditionDiscount.map(
                                  (discount, dIndex) => (
                                    <View key={dIndex}>
                                      <Text
                                        color="white"
                    ...
                    {data?.promotionType === 'FREEBIES_MIX' &&
                      data?.conditionDetail.map((detail, idx) => {
                            ...
                            <>
                              <View
                                ...
                                {detail.products.map((product, index) => (
                                  <Text color="white">
                                    {`${product.productName} ขนาด ${
                                      product.packSize
                                    }${
                                      index + 1 === detail.products.length
                                        ? ''
                                        : ','
                                    } `}
                                  </Text>
                                ...
                                (freebieDetail, idx) => (
                                  <View
                                    ...
                                    <Text
                                      ...
                                      {`แถม`}
                                      {freebieDetail.freebies.map(
                                        (freebie, idx) =>
                                          ` ${freebie.productName} จำนวน ${
                                            freebie.quantity
                                          } ${
                                            freebie.baseUnitOfMeaTh
                                              ? freebie.baseUnitOfMeaTh
                                              : freebie.saleUOMTH
                                          }${
                                            idx + 1 ===
                                            freebieDetail.freebies.length
                                              ? ''
                                              : ','
                                          } `,
                                      )}
                          ...
                          return (
                            <>
                              ...
                              <Text
                                ...
                                detail.size
                              } kg / L ${detail.conditionFreebies[0].freebies.map(
                                (freebie, idx) =>
                                  `แถม${freebie.productName} ${
                                    freebie.packSize
                                      ? `ขนาด ${freebie.packSize}`
                                      : ``
                                  } จำนวน ${freebie.quantity} ${
                                    freebie.baseUnitOfMeaTh
                                      ? freebie.baseUnitOfMeaTh
                                      : freebie.saleUOMTH
                                  }${
                                    idx + 1 ===
                                    detail.conditionFreebies[0].freebies.length
                                      ? ''
                                      : ','
                                  } `,
                              )}`}</Text>
                    ...
                    {data?.promotionType === 'DISCOUNT_NOT_MIX' &&
                      data?.conditionDetail.map((itm, idx) => {
                        return (
                          <>
                            <Text color="white">
                              ซื้อ{itm.productName} ขนาด {itm.packsize}
                            </Text>
                              ...
                              return (
                                <View key={idx}>
                                  <Text
                                    ...
                                    {`•  ${t(
                                      'screens.ProductDetailScreen.promotionDiscount',
                                      {
                                        buy: el.quantity,
                                        discountPrice: el.discountPrice,
                                        productNameFree: '' || '',
                                        unitDiscount:
                                          el.saleUnitDiscountTH ||
                                          el.saleUnitDiscount ||
                                          '',
                                        unitBuy:
                                          el.saleUnitTH || el.saleUnit || '',
                                      },
                                    )} `}
                                  </Text>
                                </View>
                    ...
                    {data?.promotionType === 'FREEBIES_NOT_MIX' &&
                      data?.conditionDetail.map((itm, idx) => {
                        return (
                          <>
                                ...
                                <View
                                  ...
                                  <Text
                                    ...
                                    {`• ซื้อครบ ${el.quantity} ${
                                      ...
                                      el2 =>
                                        ` ${el2.productName} ${
                                          el2.packSize ? el2.packSize : ''
                                        } จำนวน ${el2.quantity} ${
                                          el2.saleUOMTH ? el2.saleUOMTH : ''
                                        }`,
                                    )}`}
                                  </Text>
                                </View>
                    ...
                    <Text color="white" fontSize={14}>
                      {`•  ${t(
                        'screens.ProductDetailScreen.durationPromotion',
                        {
                          dateStart: dayjs(data?.startDate).format(
                            'DD MMMM BBBB',
                          ),
            ...
            <View style={{ flex: 1, alignItems: 'center' }}>
              <View style={{ marginTop: 50, alignItems: 'center' }}>
                ...
                />
                <Text semiBold color="text3" style={{ marginTop: 20 }}>
                  ติดตามโปรโมชันพิเศษได้เร็วๆ นี้
                </Text>
                <Text
                  semiBold
                  fontSize={16}
                  fontFamily="Sarabun"
                  color="primary"
                  style={{ marginTop: 10 }}>{`วันที่ ${dayjs(
                  data?.startDate,
