...
export default function NewsScreen({ navigation }: Props): JSX.Element {
  ...
  const fecthNewsList = async (
    sortingType: 'MOST_READ' | 'NEWEST',
    type: string,
  ) => {
    try {
      setLoading(true);
      const company = await AsyncStorage.getItem('company');
      const res: NewsInterface[] = await NewsService.getNewsList(
        company || '',
        1,
        99,
  ...
  const handleCheckbox = (id: number) => {
    const checkedCount = checkBox.reduce(
    ...
    setCheckBox(prevState =>
      prevState.map(item => {
        if (item.id === id) {
          return checkedCount > 1 || !item.checked
      ...
      <ActionSheet ref={filterNews}>
        <View
          ...
          <View style={{ flex: 1 }}>
            <View
              ...
              <Text fontSize={24} fontFamily="NotoSans">
                เรียงตาม
              </Text>
            </View>
        ...
        <View style={{ paddingHorizontal: 16 }}>
          <TouchableOpacity
            ...
            <Text color="white" fontFamily="NotoSans" fontSize={24}>
              ตกลง
            </Text>
          </TouchableOpacity>
        </View>
